...
global_config = {
    ...
    # Esempi:
    # "dom_host": tk.StringVar(), "dom_user": tk.StringVar(),
    # "dom_pass": tk.StringVar(), "dom_port": tk.StringVar(),
...
def salva_configurazione():
    ...
    try:
        ...
        messagebox.showinfo("Salvataggio riuscito", "Configurazione salvata in " + file_path)
...
def carica_configurazione():
    ...
    try:
        ...
        global_config["selected_directory"].set(config.get("selected_directory", "nessuna"))
        ...
        messagebox.showinfo("Caricamento riuscito", "Configurazione caricata da " + file_path)
    except Exception as e:
        messagebox.showerror("Errore", "Errore nel caricamento: " + str(e))


# =========================
# MENUBAR (con stato modalità in grassetto dopo "Modalità")
# =========================
...
mode_menu.add_radiobutton(label="Preparazione", variable=current_mode, value="Preparazione", command=set_mode_preparazione)
mode_menu.add_radiobutton(label="Live",         variable=current_mode, value="Live",         command=set_mode_live)
mode_menu.add_radiobutton(label="Domini / FTP", variable=current_mode, value="Domini",       command=set_mode_domini)
mode_menu.add_radiobutton(label="Correzione",   variable=current_mode, value="Correzione",   command=set_mode_correzione)
mode_menu.add_radiobutton(label="Export",       variable=current_mode, value="Export",       command=set_mode_export)
...
def _init_mode_status_entry():
    ...
    menubar.add_cascade(label=label_text, state="disabled", font=mode_status_font)
    _mode_status_index = menubar.index("end")

def _on_mode_change(*_args):
    if _mode_status_index is not None:
        menubar.entryconfig(_mode_status_index, label="Modalità: " + current_mode.get())
...
# =========================
# BARRA SUPERIORE (Nome, Modifica cartella, Directory, Aggiorna)
# =========================
...
def _modifica_cartella():
    selected = filedialog.askdirectory(title="Seleziona directory locale di lavoro (cartelle testXX)")
    if selected:
