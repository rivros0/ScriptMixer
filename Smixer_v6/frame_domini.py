...
def format_bytes(num_bytes):
    """
    Converte un numero di byte in stringa leggibile (B, KB, MB, GB, TB).
    """
    unita = ["B", "KB", "MB", "GB", "TB"]
...
def _sanitize_alunno_tag(raw):
    """
    Normalizza il tag dell'alunno:
    - converte in minuscolo
    - rimuove spazi
    - se è presente '@', prende la parte prima della @
    """
    if raw is None:
        return ""
...
def _derive_candidate_tags(raw):
    """
    A partire da una stringa (es. email o cognome_nome) genera una lista
    di possibili tag per individuare la cartella test corrispondente.
    """
...
def _list_test_dirs(base_dir):
    """
    Restituisce la lista delle sottodirectory presenti in base_dir.
    Non applica filtri: il filtro su 'test' viene gestito in chi legge.
    """
    out = []
    ...
    for name in os.listdir(base_dir):
        percorso = os.path.join(base_dir, name)
...
def _match_test_dir(test_dirs, alunno_raw):
    """
    Prova ad associare il tag alunno a una cartella test, usando varie euristiche:
    1) __testNN
    2) testNN- / testNN_
    3) fallback: la cartella contiene sia 'test' che il tag
    """
    if not test_dirs:
        return ""
    ...
    while i < len(candidates):
        ...
        for low_name, original in td_low_map.items():
            if low_name.startswith("test") and (
                low_name.endswith(pattern_dash)
                or low_name.endswith(pattern_underscore)
            ):
                return original

        i = i + 1

    # 3) Fallback più permissivo
    i = 0
    while i < len(candidates):
        tag = candidates[i]
        for low_name, original in td_low_map.items():
...
def create_frame_domini(root, global_config):
    ...
    - Crea/carica CSV (associazione alunni ↔ dominio/FTP)
    ...
    Integrazione con configurazione globale:
    - Usa global_config["selected_directory"] come base locale (cartelle testXX).
    - Usa global_config["domains_csv_path"] per ricordare l'ultimo CSV utilizzato:
      * quando si carica o crea un CSV, il percorso viene salvato qui;
      * quando una configurazione JSON viene caricata, SMX.py ripristina
        domains_csv_path e questo frame può caricarlo automaticamente.
    """
    ...
    def log(msg):
        """
        Scrive una riga nel riquadro log.
        """
    ...
    def crea_modello_csv():
        """
        Genera un CSV di esempio con intestazioni:
        cognome, dominio, ftp_user, ftp_password

        Se possibile, popola la colonna cognome ricavando un tag dalle
        cartelle locali che contengono 'test' nel nome.
        Aggiorna anche global_config["domains_csv_path"] con il percorso
        scelto, così da poterlo salvare in configurazione JSON.
        """
        ...
        try:
            ...
            if len(cognomi) > 0:
                ...
                log(
                    "Nessuna cartella utile trovata: modello creato con sole intestazioni."
                )

            # Aggiorno il path del CSV nella configurazione globale
            if "domains_csv_path" in global_config:
                valore = global_config["domains_csv_path"]
                if hasattr(valore, "set"):
                    valore.set(percorso_csv)
                else:
                    global_config["domains_csv_path"] = percorso_csv
    ...
    # ------------------------------------------------------------------
    # Carica CSV (funzione di base che lavora su un percorso noto)
    # ------------------------------------------------------------------

    def _carica_csv_da_percorso(percorso_csv, da_auto=False):
        """
        Carica il CSV indicato da percorso_csv nella tabella.
        Se da_auto è True, il log specifica che il caricamento è avvenuto
        da configurazione salvata.
        """
        ...
        if not percorso_csv or not os.path.isfile(percorso_csv):
            if not da_auto:
                messagebox.showwarning(
                    "Attenzione",
                    "Il file CSV selezionato non esiste."
                )
            return
        ...
        if da_auto:
            log(
                "File CSV '{}' caricato automaticamente da configurazione.".format(
    ...
    # ------------------------------------------------------------------
    # Carica CSV (con dialog)
    # ------------------------------------------------------------------

    def carica_csv():
        """
        Apre un file chooser per selezionare il CSV dei domini.
        Aggiorna anche global_config["domains_csv_path"] con il percorso
        scelto, in modo da poterlo salvare / ricaricare tramite SMX.py.
        """
        base_dir = global_config["selected_directory"].get().strip()
        ...
        _carica_csv_da_percorso(percorso_csv, da_auto=False)
    ...
    def aggiorna_peso_totale_ftp():
        ...
        (se esiste) all'interno della directory selezionata.
        """
        base_dir = global_config["selected_directory"].get().strip()
    ...
    def process_update_queue():
        """
        Processa la coda di aggiornamenti provenienti da thread esterni
        (ftpAgent), applicando modifiche a GUI e log.
        """
        try:
            while True:
    ...
    def scarica_tutti():
        """
        Prepara la lista dei job FTP (uno per riga della tabella) e delega
        il download al modulo ftpAgent.
        """
        base_dir = global_config["selected_directory"].get().strip()
    ...
    def analizza_somiglianze():
        """
        Esegue l’analisi delle somiglianze tra:
        - verifiche locali (cartelle test)
        - MERGE dei file scaricati da ogni dominio in 00_DominiFTP

        Usa similarity_ftp per:
        - costruire i testi di verifica e domini
        - calcolare matrici di similitudine
        - produce un riepilogo per studente nel log
        """
        base_dir = global_config["selected_directory"].get().strip()
        ...
        if not os.path.isdir(dir_ftp_base):
            messagebox.showwarning(
                "Attenzione",
                "La directory 00_DominiFTP non esiste.\nEsegui prima il download."
            )
            return
        ...
        def progress_cb(phase, current, total, name):
            """
            Callback di avanzamento usata da similarity_ftp per scrivere nel log.
            """
            if total <= 0:
                percent = 0
            else:
        ...
        if len(nomi) == 0:
            ...
            while i < len(nomi):
                ...
                if perc_shared_chars >= soglia_allerta or sim_globale >= soglia_allerta:
                    log(
                        " ⚠ Valore alto: verificare possibile riuso intenso del codice online."
                    )
                elif perc_shared_chars >= soglia_avviso or sim_globale >= soglia_avviso:
                    log(
                        " ℹ Valore moderato: possibile riuso parziale."
                    )
    ...
    def mostra_mappa():
        """
        Mostra le heatmap di similitudine calcolate da similarity_ftp.
        """
    ...
    # ------------------------------------------------------------------
    # Integrazione con SMX: refresh_domini
    # ------------------------------------------------------------------

    def _refresh_domini():
        """
        Richiamato dal pulsante "Aggiorna cartella" in SMX.
        - Aggiorna il peso complessivo di 00_DominiFTP.
        - Se la tabella è vuota ma esiste un CSV salvato in
          global_config["domains_csv_path"], prova a ricaricarlo.
        """
        aggiorna_peso_totale_ftp()

        ha_righe = len(tree.get_children()) > 0
        if ha_righe:
            return
        ...
        if "domains_csv_path" in global_config:
            valore = global_config["domains_csv_path"]
            if hasattr(valore, "get"):
                percorso_csv = valore.get().strip()
            else:
                percorso_csv = str(valore).strip()

        if percorso_csv != "" and os.path.isfile(percorso_csv):
            _carica_csv_da_percorso(percorso_csv, da_auto=True)

    if "refresh_domini" in global_config:
        global_config["refresh_domini"] = _refresh_domini

    # ------------------------------------------------------------------
    # All'avvio: se domains_csv_path è già valorizzato (da config JSON),
    # prova a caricare automaticamente il CSV corrispondente.
    # ------------------------------------------------------------------
    ...
    if "domains_csv_path" in global_config:
        valore = global_config["domains_csv_path"]
        if hasattr(valore, "get"):
            iniziale_csv = valore.get().strip()
        else:
            iniziale_csv = str(valore).strip()

    if iniziale_csv != "" and os.path.isfile(iniziale_csv):
        _carica_csv_da_percorso(iniziale_csv, da_auto=True)

    # Aggiorna comunque il peso totale, nel caso in cui 00_DominiFTP
    # esista già al momento dell'apertura del frame.
    aggiorna_peso_totale_ftp()
