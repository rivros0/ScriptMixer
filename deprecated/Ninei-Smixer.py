...
def create_mix_file(base_directory, subdir, prompt_string, extension, output_directory):
    ...
    try:
        ...
        with open(mix_file_path, "w", encoding="utf-8", errors="replace") as mix_file:
            ...
            for file_path in files_to_mix:
                try:
                    ...
                    mix_file.write(f"Errore nella lettura di {file_path}: {str(e)}\n")
...
def merge_all_files():
    ...
    try:
        with open(merged_file_path, "w", encoding="utf-8", errors="replace") as merged_file:
            for file_name in sorted(os.listdir(output_directory)):
                if file_name.endswith("_mix.txt"):
                    ...
                    try:
                        ...
                        report_text.insert(tk.END, f"Errore nella lettura di {file_path}: {str(e)}\n")
